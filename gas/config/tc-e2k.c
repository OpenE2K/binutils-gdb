...
/* This functionality used to be enabled by no longer supported
   --fix-lcc-bug146811 option (see Bug #146811, Comments #11, #15).  */
static int use_old_encodings_for_quads = 0;
...
s_e2k_non_target_label (int ignore ATTRIBUTE_UNUSED)
{
  ...
  if (mcpu >= 7)
    {
      as_bad (_(".non_target_label directive is not supported for "
		"elbrus-v{X>=7}"));
      return;
    }
...
s_e2k_fix_lcc_bugs_142105_149113 (int idx)
{
  ...
  if (mcpu >= 7)
    {
      as_bad (_(".fix_lcc_bug%s directive is not supported for "
		"elbrus-v{X>=7}"), names[idx]);
      return;
    }
...
const pseudo_typeS md_pseudo_table[] =
{
  ...
  {"uahalf", s_uacons, 2},
  /* Implement `.half' directive symmetrically to `.uahalf' (Bug #161145,
     Comment #2).  */
  {"half", cons, 2},
  {"uaword", s_uacons, 4},
  {"uadword", s_uacons, 8},
...
md_parse_option (int c, const char *arg)
{
  switch (c)
      ...
      else if (strcmp (arg, "elbrus-v7") == 0)
	{
      ...
      else if (strcmp (arg, "elbrus-48c") == 0)
	{
	  mcpu = 7;
	  output_mach = bfd_mach_e2k_48c;
	  forward_incompat = 1;
	}
      else if (strcmp (arg, "elbrus-8v7") == 0)
	{
	  mcpu = 7;
	  output_mach = bfd_mach_e2k_8v7;
	  forward_incompat = 1;
	}
      else if (strcmp (arg, "elbrus-maket32c") == 0)
	{
	  mcpu = 7;
	  output_mach = bfd_mach_e2k_maket32c;
	  forward_incompat = 1;
	}
      else
      ...
      if (arg)
	{
	  ...
	  /* Otherwise this option specifies the "size" to use in the 1st
	     variant of the workaround. */
		   ...
		   && mcstbug_140436_size != 2048
		   && mcstbug_140436_size != 1024)
	    as_bad (_("invalid \"%s\" value of -fix-lcc-bug142105: "
...
e2k_after_parse_args (void)
{
  ...
  if (mcpu >= 7)
	...
	{
	  as_bad (_("--fix-lcc-bug142105 with non-zero value is not "
		    "supported for elbrus-v{X>=7}"));
	  return;
	}
...
typedef struct {
  ...
  /* Either of {I,R}BRANCH met in the current WI.  */
  int irbranch_met;
} wide_command;
...
parse_pred_generic (char **pstr, e2k_pred *ppred, int global, int allow_pcnt)
{
  ...
  do {
    ...
    if (global)
      {
	if (ppred->pred_num < 0 || ppred->pred_num > 31)
	  {
	    /* Do NOT update *PSTR as the existing outer error handlers refer
	       to the troublesome predicate right in the source line. They do
	       NOT rely on ppred->PRED_NUM and ppred->PRED_FLD, which is why
	       they are left uninitialized here (note that an invalid
	       %pcnt{X>=32} could result in the emergence of an inappropriate
	       0x20 bit in ppred->PRED_FLD and thus be mistakenly taken for
	       %pred{X-32}.  */
	    return 0;
	  }
  ...
  for (i = 0; i < free_als->real_als_nmb; i++)
    {
      ...
      if ((free_als->real_als_nmb == 2
	   /* Single-ALC encodings of quad instructions require %empty.lo as
	      well as two-ALC ones in their lower ALC<j> (Bug #147272).  */
	   || free_als->implicit_paired_alc_delta != 0)
          && fmt == QUAD
          && (arg_idx != 1
...
parse_dst (char **pstr, e2k_register_format fmt)
{
  ...
  if (parse_generic_register (&s, &reg))
  ...
  else
    {
	 ...
	 `%empty.{lo,hi}' destinations into account in such a stupid way.  */
      if (free_als->real_als_nmb == 2
	  /* Single-ALC encodings of quad instructions require %empty.lo as
	     well as two-ALC ones in their lower ALC<j> (Bug #147272).  */
	  || free_als->implicit_paired_alc_delta != 0)
...
static int
parse_three_args_simple (char **pstr,
			 const e2k_register_format *arg_fmt)
{
  do {
    char *s = *pstr;

    CHECK (parse_src1 (&s, arg_fmt[0]));
    CHECK_MSG (slurp_char (&s, ','), _("comma expected before `%s'"), s);
    CHECK (parse_src2 (&s, arg_fmt[1]));
    CHECK (parse_dst (&s, arg_fmt[2]));
...
{
  if (!for_load && !for_store)
    return parse_three_args_simple (pstr, arg_fmt);
...
parse_alf1_args (char **pstr, const e2k_opcode_templ *op)
{
  ...
  do {
    ...
    /* FIXME: stupidly treat ADD*, GETPTR*, GETIND and GETMI instructions as
       LD*-like ones from the point of view of processing of their arguments
       for now.  */
    CHECK (parse_three_args_for_ld_st (&s, alf1->arg_fmt,
				       (((op->name[0] == 'l'
					  && op->name[1] == 'd')
					 || (op->name[0] == 'a'
					     && op->name[1] == 'd'
...
parse_mova_args (char **pstr, const e2k_opcode_templ *op)
{
  ...
  /* This is MOVAQ and it needs to be encoded in a pair of AAS syllables.  */
  int is_paired_movaq = is_movaq && (mcpu < 7
				     || use_old_encodings_for_quads != 0);
  ...
  /* Mark the paired AAS for MOVAQ as busy no matter if it will explicitly be
     encoded so that other instructions may not occupy it.  */
  if (is_movaq)
    wc.busy_aa2f1 |= 1 << (chn + 1);


  /* Set `AA2F1.opc'.  */
  for (i = 0; i < (is_paired_movaq ? 2 : 1); i++)
  ...
  for (i = 0; i < 16; i++)
      ...
      if (seen[k])
	{
	  ...
	  for (j = 0; j < (is_paired_movaq ? 2 : 1); j++)
  ...
  if (is_movaq)
    {
      ...
      wc.aa2f1[chn] |= (big_endian ? ind + 8 : ind) << param[1].bit_offset;

      if (is_paired_movaq)
	wc.aa2f1[chn + 1] |= ((big_endian ? ind : ind + 8)
			      << param[1].bit_offset);
    }
  ...
  if (parse_generic_register (&s, &reg))
  ...
  else
    {
      ...
      /* This presumably enforces the use of the LO EVEN destination register
	 half if the user specified the HI ODD one somehow.  */
      if (is_movaq)
        special_dst &= 0xfe;
...
#undef fapb_fields
  ...
  if (mcpu >= 2 && si_fld.set)
    {
      as_bad (_("`si' flag is not supported in fapb starting from elbrus-v2"));
      return 0;
    }

  /* See iset upgrade u7.32 and Bug #148692.  */
  if (mcpu >=7 && dpl_fld.set)
    {
      as_bad (_("`dpl' flag is not supported in fapb starting from elbrus-v7"));
      return 0;
    }
  ...
  if (ct_fld.set)
    ...
    {
      /* On elbrus-v7 FAPB with no "ct" flag (note that "dpl" is no longer
	 supported, see above) is equivalent to `FAPB dpl=0' on
	 elbrus-v{X<6}. This is the only alternative now that it is impossible
	 to specify "dpl=1".  */
      chn = 1;

      /* Just to make the code more explicit: the default dpl_fld.val is
	 already zero and ...  */
      dpl_fld.val = 0;
      /* ... even if it was left UNSET, the matching bits in APSx are zeroed
	 out as a part of the whole Wide Instruction in `start_wide_command
	 ()'.  */
      dpl_fld.set = 1;
    }
  else
...
parse_alf5_args (char **pstr, const e2k_opcode_templ *op)
{
  ...
  do {
    ...
    free_als->ales.alef2.opce = NONE_VALUE;
    free_als->ales.alef2.opc2 = EXT_VALUE;
    free_als->u[0].alf5.opce = alf->opce;
...
parse_alf6_args (char **pstr, const e2k_opcode_templ *op)
{
  ...
  do {
    ...
    free_als->ales.alef2.opce = NONE_VALUE;
    free_als->ales.alef2.opc2 = EXT_VALUE;
    free_als->u[0].alf6.none = alf->src2;
...
parse_alf7_args (char **pstr, const e2k_opcode_templ *op)
{
  ...
  do {
    ...
    CHECK_MSG (slurp_char (&s, ','), _("comma expected at `%s'"), s);
    CHECK_MSG (parse_pred (&s, &pred, 0), _("a valid predicate expected at `%s'"), s);
    ...
    *pstr = s;
    ...
    /* Take comparison operations with an explicitly specified ALC into account.
       Note that only _integer_ comparison operations should be tracked (read
       C.17.1.2 attentively and see Bug #148965, Comment #2). To make it
       possible to distinguish them from floating-point comparisons they are
       marked by `e2k-opcode-generator' via INT_COMPARISON field.

       The results of ALOPF8 `ccto*' operations shouldn't be referred to by
       `%cmpX' conditions as well. Therefore, they are not tracked in
       `parse_alf8_args ()'.
       ...
       why.  */
    if (alf->int_comparison && free_als->plcmnt_nmb == 1)
      {
...
parse_alf_args (char **pstr, const e2k_opcode_templ *op)
{
  ...
  char *s;
  int old_quad_encodings = mcpu < 7 || use_old_encodings_for_quads;
  ...
  if ((alf->alopf == ALOPF11
    ...
    {
      free_als->real_als_nmb = old_quad_encodings ? 2 : 1;
                 ...
                 const e2k_opcode_templ *op ATTRIBUTE_UNUSED)
{
  static const char *param[16] = {
    ...
    /* These ones have emerged starting from elbrus-v7.  */
    "macp=", "fl_c_mode="
  };
  ...
  do
    {
	 ...
	 combinations of WAIT parameters, not just the first one.  */
      ...
      /* `fl_c_mode = ...' presumably makes no sense without `fl_c = 1'.  */
      if (val[15] != -1 && val[4] != 1)
	{
	  as_bad (_("`fl_c_mode = ...' makes no sense without `fl_c = 1'"));
	  invalid_comb = 1;
	}
  ...
  do
    {
      ...
      /* According to C.17.1.2 in iset-v7.single control condition with
	 `ctcond.ct == 11' should be applied only to IBRANCH{,D}, otherwise,
	 ct_cond is considered to be FALSE (FIXME(?):`ibranch == IBRANCH'
	 argument is currently provided to this function for both of them).  */
      else if (mcpu >= 7 && ibranch == IBRANCH && parse_pl_ctcond (&s, &psrc))
	{
	  SET_FIELD (ss, ctcond, ((11 << 5) | psrc), 0);
	}
  ...
  do
    {
      ...
      if (wc.ss.set.ctop)
        {
          as_bad (_("cannot encode `%s', SS.ctop in the current wide "
		    "instruction is already busy"), op->name);
          break;
        }

      // wc.ss.val.fields.ctop = ctprn;
      SET_FIELD (ss, ctop, ctprn, 0 /* non-default  */);
...
tc_e2k_fix_adjustable (struct fix *fixP)
{
  ...
  if (fixP->fx_r_type == BFD_RELOC_SIZE32
	  ...
	  && (fixP->fx_r_type == BFD_RELOC_E2K_AP
	      /* Avoid the replacement of local symbols with sections in PM for
		 these relocations so as to give control that data is not
		 allocated in CUD (not supported yet) and code is not allocated
		 in GD (makes no sense) a chance.  */
	      || fixP->fx_r_type == BFD_RELOC_32
	      || fixP->fx_r_type == BFD_RELOC_E2K_64_ABS_LIT)))
    return 0;
     ...
     fixS *fixp;
{
 ...
 if (rel->howto == NULL)
    {
      /* This may happen when a relocation of unsupported bitness is required
	 for the evaluation of a symbolic expression. For example, when an
	 inappropriate directive like `.short' is used to accomodate a symbol
	 (Bug #145733, Comment #6).  */
      as_bad_where (fixp->fx_file, fixp->fx_line,
		    _("cannot represent relocation type %s"),
		    bfd_get_reloc_code_name (fixp->fx_r_type));
      /* Set howto to a garbage value so that we can keep going without getting
	 SIGSEGV at a later stage.  */
  ...
  if (plcmnt->exp.X_op == O_constant)
  ...
  else if (plcmnt->exp.X_op == O_tls_le)
    {
      ...
      switch (size)
	   ...
	   /* All relocations referencing an entry in .got are resolved with a
	      32-bit value in all modes including PM for elbrus-v7.  */
  ...
  for (i = 0; i < 4; i++)
    {
	  ...
	  /* This extra test ensures that the respective AA2F1 syllable is
	     actually present in the WI (it has become necessary starting from
	     elbrus-v7 where a "paired" AA2F1 may be just reserved, but not
	     present in the WI) as it may not remain zero in such a case
	     because of non-zero values of AAS.opc fields (see Table B.7.1).
	     This notion saves me the trouble of introducing a separate
	     wc.present_aa2f1[] or something like that.  */
	  && wc.aa2f1[i] != 0)
        {
          memcpy (hsyllables + (hsyllable_cntr & ~0x1) + 1
                  - (hsyllable_cntr & 0x1),
...
encode_setr0 (int setup_opcode)
{
  ...
  if ((wc.setcmd_args[SETWD_IDX] & ~setcmd_masks[SETWD_IDX]) == 0
	...
	lts.fields.wsz = 0;
      ...
      /* Is mcn explicitly specified? */
      if ((wc.setcmd_args[SETWD_IDX + 3] & ~setcmd_masks[SETWD_IDX + 3]) == 0)
        lts.fields.mcn = (wc.setcmd_args[SETWD_IDX + 3]
                          & setcmd_masks[SETWD_IDX + 3]);
      else
        /* See if mcn field should implicitly be set to 0.  */
        lts.fields.mcn = 0;
    }
...
end_wide_command (void)
{
  do {
    ...
    /* See Bug #154538, Comments #0, #6 and #8.  */
    if (mcpu <= 5
	&& wc.ss.started && wc.ss.set.ctop
	&& wc.ss.val.fields.ctop != 0
	&& wc.cs0.set
	&& wc.cs0.val.fields.ctpr == wc.ss.val.fields.ctop)
      as_bad (_("the wide instruction contains CT and CTP instructions "
		"making use of the same %%ctpr register (%%ctpr%d) which "
		"is not supported on elbrus-v%d"),
	      (int) wc.ss.val.fields.ctop, (int) mcpu);
	...
	if (ct == 11)
	  {
	    /* According to C.17.1.2 this control-transfer condition is
	       equivalent to FALSE in the presence of any of A{LC,BN,BP}
	       instructions with differing TRUE and FALSE flags (Bug #149213).
	       It's likely(?) that this requirement applies to each of the
	       aforementioned instructions separately from other ones: the
	       values of flags may very well differ between them.  */

	    /* ALC{F,T} should not be set separately from each other.  */
	    gas_assert ((!wc.ss.set.alcf && !wc.ss.set.alct)
			|| (wc.ss.set.alcf && wc.ss.set.alct));
	    if (wc.ss.set.alcf && wc.ss.set.alct
		&& wc.ss.val.fields.alcf != wc.ss.val.fields.alct)
	      as_bad (_("control-transfer condition is equivalent to false "
			"because of differing ALC.alc{f,t} in the containing "
			"wide instruction"));

	    /* ABN{F,T} should not be set separately from each other.  */
	    gas_assert ((!wc.ss.set.abnf && !wc.ss.set.abnt)
			|| (wc.ss.set.abnf && wc.ss.set.abnt));
	    if (wc.ss.set.abnf && wc.ss.set.abnt
		&& wc.ss.val.fields.abnf != wc.ss.val.fields.abnt)
	      as_bad (_("control-transfer condition is equivalent to false "
			"because of differing ABN.abn{f,t} in the containing "
			"wide instruction"));

	    /* ABP{F,T} should not be set separately from each other.  */
	    gas_assert ((!wc.ss.set.abpf && !wc.ss.set.abpt)
			|| (wc.ss.set.abpf && wc.ss.set.abpt));
	    if (wc.ss.set.abpf && wc.ss.set.abpt
		&& wc.ss.val.fields.abpf != wc.ss.val.fields.abpt)
	      as_bad (_("control-transfer condition is equivalent to false "
			"because of differing ABP.abp{f,t} in the containing "
			"wide instruction"));
	  }
      }
...
{
  /* Unlike md_assemble () this hook is invoked even if the current input is
     not to be assembled (for example, surrounded by `.if 0 ... .endif').
     Nothing is probably to be done here in such a case for our purposes. The
     failure to realize this from the very beginning has resulted in the
     emergence of redundant `{nop}'s for each explicitly specified `{...}'
     wide instruction under `.if 0' (Bug #150881).  */
  if (ignore_input ())
    return;
...
int
parse_dtbp_args (char **pstr, const e2k_opcode_templ *op ATTRIBUTE_UNUSED)
{
  if (! wc.irbranch_met)
    {
      as_bad (_("no {i,r}branch precedes dtbp in WI"));
      return 0;
    }

  gas_assert (wc.ss.started);

  if ((*pstr)[0] != '\0')
    {
      as_bad (_("dtbp does not expect any arguments, but `%s' passed"), *pstr);
      return 0;
    }

  SET_FIELD (ss, dtbp, 1, 0);
  return 1;
}
  ...
  if (*nextcharP ==  '@')
    {
	  ...
	  || ((next_end = e2k_end_of_match (&input_line_pointer[1], "DYNOPT"))
              && (type = O_dynopt))
	  || ((next_end = e2k_end_of_match (&input_line_pointer[1], "PCREL"))
	      && (type = O_pc_lit))
	  )
        {
