...
#define N(MACH, MODE, PRINT, DEFAULT, NEXT) \
  __N (bfd_arch_e2k, MACH, MODE, PRINT, DEFAULT, NEXT)
...
const bfd_arch_info_type bfd_e2k_golang_arch =
  __N (bfd_arch_e2k_golang,
       /* This combination results in `4 * bfd_mach_e2k_ev2'
	  machine number.  */
       bfd_mach_e2k_ev2, 64,
       "elbrus-v2-golang",
       /* This is the only entry for bfd_arch_e2k_golang, so it should probably
	  be considered default. This value ensures that this entry will be
	  accepted by `bfd_lookup_arch (..., machine = 0)' despite the fact
	  that its (and the only currently supported) "machine number" (see
	  a few lines above) is different from 0.  */
       true,
       /* This is the trailing entry.  */
       NULL);
