...
adjust_offset_in_cud_gd (struct bfd_link_info *info, bfd_vma offset)
{
  ...
  if (htab->ranges == NULL)
	   ...
	   m = m->next, p++)
	{
      ...
      cud_size = gd_size = 0;
      max_page_aligned[0] = max_page_aligned[1] = 0;
      for (i = 0; i < num; i++)
	{
	  ...
	  if (min_page_aligned < max_page_aligned[ranges[i].in_cud ? 0 : 1])
	    {
	  ...
	  max_page_aligned[ranges[i].in_cud ? 0 : 1]
	    = (ranges[i].max + 0xfffULL) & ~(0xfffULL);
...
#define AT_ev7 (AT (e48c) | AT (e8v7))
  ...
  if (protected_mode)
    {
	 ...
	 a single elbrus-v7 input object (see Bug #143326, Comments #7
	 and #8).  */
      if (already_invoked)
	{
	  if ((omach & (AT_ev7 | AT (ev8))) != omach)
	    omach &= ~(AT_ev7 | AT (ev8));
	}
      else
	already_invoked = 1;
  ...
  _bfd_error_handler
	...
	may have no relation to extra control implemented in Bug #143326) to be
	on the safe side.  */
...
bool
_bfd_e2k_elf_late_size_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				 struct bfd_link_info *info)
{
  struct _bfd_e2k_elf_link_hash_table *htab;
  bfd *dynobj;
      ...
      if (i->size > 0)
        {
	      ...
	      || bfd_write (wei->contents, i->size, abfd) != i->size)
            {
              wei->res = false;
              return;
            }
			  ...
			  struct pm_range *ranges)
{
  ...
  bfd_vma max_page_aligned[2] = {0, 0};
