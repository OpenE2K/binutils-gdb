...
info_shared_base_command (const char *pattern, int from_tty)
{
  ...
  const struct solib_ops *ops = gdbarch_so_ops (gdbarch);
  struct ui_out *uiout = current_uiout;
  ...
  for (solib &so : current_program_space->solibs ())
	...
	continue;
      ...
      /* ADDR = ENDADDR = 0, THE_BFD_SECTION = &dummy, OWNER = nullptr. The
	 latter shouldn't matter at all since `base_section' is used in a
         rather specific way.  */
      struct target_section base_section (0, 0, &dummy);
