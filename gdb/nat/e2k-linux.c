...
{
  /* Encode `GETSAP "-input_function_args_size", %qr0.  */
  prog[2] &= 0xffffffff00000000ULL;
  prog[2] |= getsp & 0xffffffffULL;

  /* Encode `GETSAP "-rval_on_stack_size", %qb[2]'. Hopefully, this won't
     change %sp in case RVAL is to be returned on registers (rval_on_stack_size
     == 0). As for %qb[2] it will be-reset with the first "user" argument in
     such a case.  */
  /* NOTE that currently the return value is read by `e2k_return_value ()'
     starting from %sp of the DUMMY frame, therefore, it's crucial that the
     allocation of this area is done AFTER the one intended for input arguments
     (whereas from the point of view of the called PM function the order of
     allocations should make  no difference and the compiler may perform them
     vice versa). */
  prog[4] &= 0x00000000ffffffffULL;
  prog[4] |= getsp & 0xffffffff00000000ULL;
}
...
static const struct e2k_insn_seq dummy_prologue_pm =
  {
    ...
    /*                    ALS1     0x62ecd981,  */
    /*                    CS1      0x040001c1,     setwd wsz = 0x9, nfx = 0x1  */
    /*                                             setbn rsz = 0x7, rbs = 0x1, rcur = 0x0  */
    /*                    LTS1     0xdeadbeef,  */
    /*                    LTS0     0x00000130,  */

    /* 0000<000000000000> HS	   0x0c000012,
                          ALS0     0x62ecd802,     getsap,0 _f32s,_lts0 0xffffffc0, %qb[2]
			  ALS1     0x62ecd803,
			  LTS0     0xffffffc0,
    */
    ...
    */

    {
      ...
      0x040001c162ecd981ULL,
      0x00000130deadbeefULL,

      0x62ecd8020c000012ULL,
      0xffffffc062ecd803ULL,
...
e2k_linux_call_target (int tid, int arch_idx, ULONGEST pc)
{
  ...
  /* Dummy frame has `WD.size_q == 5' in "ordinary" modes and 9 in PM, thus
     CALL the target function with `wbs = 0x1' to match the relevant ABI.  */
