  ...
  if (reg)
    {
	  ...
	  /* Experimental "post elbrus-v6" corefiles (note that our kernel
	     hackers are continuously extending them nowadays) are to be
	     treated as regular "elbrus-v6" ones in line with the approach
	     taken in `e2k_linux_supply_core_gregset ()'. For our limited
	     purposes here it is sufficient that they contain %idr at the
	     same offset in their extended user_regs_struct.  */
...
void
e2k_linux_report_signal_info (struct gdbarch *gdbarch,
			      struct ui_out *uiout,
			      enum gdb_signal siggnal)
{
  /* This shouldn't have been borrowed from i386 as on e2k we need to report
     additional signal info not just for SIGSEGV.  */
#if 0
...
void _initialize_e2k_linux_tdep ();
void
_initialize_e2k_linux_tdep ()
{
