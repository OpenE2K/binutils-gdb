...
/* Linux target op definitions for the E2K architecture.  */

class e2k_target : public linux_process_target
{
public:
...
static e2k_target the_e2k_target;
...
CORE_ADDR
e2k_target::low_get_pc (regcache *regcache)
{
  CORE_ADDR pc;
...
static struct regs_info regs_info_e2k =
  {
    NULL,
    NULL,
    &e2k_regsets_info 
...
const regs_info *
e2k_target::get_regs_info ()
{
  return &regs_info_e2k;
}
...
const gdb_byte *
e2k_target::sw_breakpoint_from_kind (int kind, int *size)
{
...
bool
e2k_target::low_breakpoint_at (CORE_ADDR where)
{
...
bool
e2k_target::supports_z_point_type (char z_type)
{
  switch (z_type)
    {
#if 0
...
int
e2k_target::low_insert_point (raw_bkpt_type type, CORE_ADDR addr,
...
int
e2k_target::low_remove_point (raw_bkpt_type type, CORE_ADDR addr,
			      int size, raw_breakpoint *bp)
{
  struct process_info *proc = current_process ();
...
bool
e2k_target::low_stopped_by_watchpoint ()
{
  struct process_info *proc = current_process ();
...
CORE_ADDR
e2k_target::low_stopped_data_address ()
{
  struct process_info *proc = current_process ();
  CORE_ADDR addr;
...
void
e2k_target::low_delete_process (arch_process_info *info)
{
  xfree (info);
}

void
e2k_target::low_prepare_to_resume (lwp_info *lwp)
{
  e2k_linux_prepare_to_resume (lwp);
}

bool
e2k_target::low_cannot_store_register (int regno)
{
  return (regno >= 1234);
}

bool
e2k_target::low_cannot_fetch_register (int regno)
{
  return (regno >= 1234);
}
...
linux_process_target *the_linux_target = &the_e2k_target;
