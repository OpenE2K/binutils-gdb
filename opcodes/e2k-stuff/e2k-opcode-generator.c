		      ...
		      const char *parse_args, int min, int max)
{
  ...
  printf ("{\n\tstatic %s dummy =", type);
  printf ("\n\t\t{\"%s\", 0x%02x, %s", name, iset_mask, parse_args);
...
void
print_common_part (const char *type, const char *name,
		   const char *parse_args)
{
  /* Note that elbrus-v1 "forward-compatible" instructions are generic ones and
     ...
     really should.  */
  print_common_part_ex (type, name, parse_args, mcpu,
			forward_incompat ? mcpu : 8);
}

void
print_common_part_with_cpu (int cpu, const char *type, const char *name,
			    const char *parse_args)
{
  int saved_cpu = mcpu;
  mcpu = cpu;
  print_common_part (type, name, parse_args);
  mcpu = saved_cpu;
}
...
e2k_oper_table oper_tables[][5] =
  {
    ...
    {
      {
        sizeof (elbrus_v7_short_opers) / sizeof (e2k_oper),
        elbrus_v7_short_opers
      },

      {
        sizeof (elbrus_v7_long_opers) / sizeof (e2k_oper),
        elbrus_v7_long_opers
      },

      {
        sizeof (elbrus_v7_long_ext1_opers) / sizeof (e2k_oper),
        elbrus_v7_long_ext1_opers
      },

      {
        sizeof (elbrus_v7_long_ext2_opers) / sizeof (e2k_oper),
        elbrus_v7_long_ext2_opers
      },
      ...
      for (j = 0; j < alopf1_groups[i].size; j++)
        {
          ...
          print_common_part ("e2k_alf1_opcode_templ", crnt->name,
			     "parse_alf_args");
  ...
  size_t i;
  {
    ...
    for (i = 0; i < sizeof (custom) / sizeof (custom[0]); i++)
      {
        ...
        print_common_part ("e2k_alf2_opcode_templ", custom[i].name,
			   "parse_alf_args");
    ...
    for (i = 0; i < 2; i++)
      {
        ...
        for (j = 0; j < 4; j++)
          {
            print_common_part ("e2k_alf2_opcode_templ",
...

    {
      ...
      if (fetch_cop_channels (short_opers, "getpl", &cop_chn))
        {
          print_common_part ("e2k_alf2_opcode_templ", "getpl",
			     "parse_alf_args");
          print_alf_part ("ALOPF2", "NO_MAS", cop_chn.cop, cop_chn.channels,
                          "ARGS_SD");
      ...
      if (fetch_cop_channels (short_opers, "drtoap", &cop_chn))
        {
          ...
          print_common_part ("e2k_alf2_opcode_templ", "getsap",
			     "parse_alf_args");
          print_alf_part ("ALOPF2", "NO_MAS", cop_chn.cop, cop_chn.channels,
                          "ARGS_SQ");
...

    {
	  ...
	  const char *names[][2] = {{"cudtoap", "gdtoap"},
				    {"cudincr", "gdincr"}};
	  int isets[][2] = {{1, 6}, {7, 8}};
	  const char *fmts[2] = {"ARGS_SQ", "ARGS_DQ"};
	      ...
	      for (j = 0; j < 2; j++)
		{
		  print_common_part_ex
		    ("e2k_alf2_opcode_templ", names[i][j],
		     "parse_alf_args", isets[i][0], isets[i][1]);
		  print_alf_part ("ALOPF2", "NO_MAS", cop_chn.cop,
				  cop_chn.channels, fmts[i]);
		  /* ALF2.opce == GD == 0xf2 (see Table B.5.1)  */
		  printf (", 0x%x", alf2_opce[j]);
		  print_final_part ();
		}
            }
        }
  ...
  for (i = 0; i < sizeof (gen_ins) / sizeof (gen_ins[0]); i++)
    {
	  ...
	  /* Those comparisons among ALOPF7 ones not starting with 'f' (and
	     thus starting with 'c' as there are no `{,q}pcmp*'s among them)
	     are likely to be really integer ones.  */
	  /* .int_comparison = ...  */
	  if (ins_name[i][j][0] != 'f')
	    {
	      /* Just a sanity check.  */
...
gen_alopf17 ()
{
    ...
    for (i = 0; i < 2; i++)
    {
	  ...
	  /* However, it's likely that there are NO integer comparisons among
	     ALOPF17 ones as all of them are `{,q}pCMP*'s which are not
	     considered to be integer (see Bug #148965, Comment #2).  */
	  /* .int_comparison = 0  */
	  printf (", 0");
  ...
  for (j = 0; j < 2; j++)
    {
      for (i = 0; i < groups[j].size; i++)
        {
          instr_cop_chn cop_chn;
	  const char *cop_name = groups[j].loads[i].name;
	  const char *opce = "NONE";
	  int real_cpu = mcpu;

	  if (strcmp (cop_name, "ldosrrqp") == 0)
	    {
	      real_cpu = 7;
	      cop_name = "ldrqp";
	      opce = "0xdc";
	  ...
	  print_common_part_with_cpu (real_cpu,
          ...
          print_alopf11_tail (opce, cop_chn.channels,
                              j == 0 ? "EXT" : "EXT1", 0);
          print_final_part ();
        }
    }
...
gen_alopf11 ()
{
  ...
  for (i = 0; i < 2; i++)
    {
	  ...
	  if (strcmp (crnt->name, "getmi") == 0
	    ...
	    cop_name = "getptr";
	  else if (strcmp (crnt->name, "qpsbgltrlo") == 0
		   || strcmp (crnt->name, "qpsbgltrhi") == 0)
	    cop_name = "qpsbgltr";
	  else if (strcmp (crnt->name, "subarrc") == 0)
	    cop_name = "subarr";
	  ...
	  if (strcmp (crnt->name, "getind") == 0
		 ...
		 for a few other instructions. Other ones refer to it via
		 distinct names or just the immediate value.  */
		...
		= (k == 0
	      ...
	      if (! fetch_cop_channels (lopers, cop_name, &cop_chn))
                continue;
	      ...
	      /* According to a note below Table B.1.3.2 in iset-v7.single
		 QPSBGLTR may be encoded in ALC1 only.  */
	      else if (strcmp (cop_name, "qpsbgltr") == 0)
		{
		  if (cop_chn.channels[4] == 0)
		    abort ();
		  cop_chn.channels[4] = 0;
		}

	      print_common_part_with_cpu
		(real_cpu,
		 (need_extra_ind
		  ? "e2k_alopf11_lit8_opcode_templ"
		  : "e2k_alopf11_opcode_templ"),
		 crnt->name, "parse_alf_args");
	      ...
	      print_alopf11_tail (opce, cop_chn.channels, ext,
                                  explicit_ales25_v4);
        ...
        for (j = 0; j < 8; j++)
          {
            ...
            print_common_part ("e2k_alopf11_opcode_templ", name,
			       "parse_alf_args");
     ...
     for (i = 0; i < 5; i++)
       {
         ...
         for (j = 0; j < 2; j++)
           {
             print_common_part ("e2k_alopf11_opcode_templ",
  ...
  {
    static const char *generic[] = {"qps2m", "qps2cm"};
    static const char sfx[] = {'b', 'h', 'w', 'd'};
    static const char *opce[] = {"QPB", "QPH", "QPW", "QPD"};
    instr_cop_chn cop_chn;
	...
	int j;
	char custom[10];
	char *end = stpcpy (custom, generic[i]);

	if (! fetch_cop_channels (long_ext1_opers, generic[i], &cop_chn))
	  continue;
	...
	for (j = 0; j < 4; j++)
	  {
	    ...
	    /* QPS2CMB was claimed redundant (see Bug #138945 and a relatively
	       recent version of iset-v7.single).  */
	    if (i == 1 && j == 0)
	      continue;

	    print_common_part ("e2k_alopf11_opcode_templ", custom,
			       "parse_alf_args");
	    print_alf_part ("ALOPF11", "NO_MAS", cop_chn.cop,
			    cop_chn.channels, "ARGS_PPS");
	    print_alopf11_tail (opce[j], cop_chn.channels, "EXT1", 0);
	    print_final_part ();
	  }
      }
  }

  {
    ...
    if (fetch_cop_channels (long_ext2_opers, "qptoqp2", &cop_chn))
      {
	/* More custom instructions may presumably emerge here as Table B.3.2.2
	   is filled in. Amazingly, they have probably got no name for
	   `ALES.opce == 0xce' encoding used below.  */
	print_common_part ("e2k_alopf11_opcode_templ", "qpfstobf",
			   "parse_alf_args");
	print_alf_part ("ALOPF11", "NO_MAS", cop_chn.cop, cop_chn.channels,
			"ARGS_PPP");
	print_alopf11_tail ("0xce", cop_chn.channels, "EXT2", 0);
	print_final_part ();
      }
  }

  {
    instr_cop_chn cop_chn;
    char name[16] = "qpack";
    if (fetch_cop_channels (long_ext2_opers, name, &cop_chn))
      {
	int i;

	/* See Table B.3.3 on custom instructions' names and encodings.  */
	name[8] = 's';
	name[9] = '\0';

	for (i = 0; i < 4; i++)
	  {
	    static const char *sfx[] = {"hbs", "hbu", "whs", "whu"};
	    /* They don't refer to any special names for these encoding in
	       Table B.3.3.  */
	    static const char *ales_opce[] = {"0xc0", "0xc1", "0xc2", "0xc3"};
	    name[5] = sfx[i][0];
	    name[6] = sfx[i][1];
	    name[7] = sfx[i][2];
	    ...
	    print_alf_part ("ALOPF11", "NO_MAS", cop_chn.cop, cop_chn.channels,
			    "ARGS_PPP");
	    print_alopf11_tail (ales_opce[i], cop_chn.channels, "EXT2", 0);
	    print_final_part ();
	  }
      }
  }
...
gen_alopf12 ()
{
  ...
  for (i = 0; i < sizeof (usd_instrs) / sizeof (usd_instrs[0]); i++)
    {
      ...
      /* GETSP may be encoded in ALC0 only (excluding ALC3), while GETSOD in a
	 pair of ALC{0,1} (excluding ALC{3,4}). This adjustment ensures the
	 right result for both operations.  */
      cop_chn.channels[3] = cop_chn.channels[4] = 0;
  ...
  {
    static const char *generic = "qpcm2";
    static const char sfx[] = {'b', 'h', 'w', 'd'};
    static const char *ales_opce[] = {"QPB", "QPH", "QPW", "QPD"};
    instr_cop_chn cop_chn;
    char custom[10];
    char *end = stpcpy (custom, generic);

    if (fetch_cop_channels (long_ext1_opers, generic, &cop_chn))
      {
	int j;
	for (j = 0; j < 4; j++)
	  {
	    ...
	    print_common_part ("e2k_alopf12_opcode_templ", custom,
			       "parse_alf_args");
	    print_alf_part ("ALOPF12", "NO_MAS", cop_chn.cop, cop_chn.channels,
			    "ARGS_SP");
	    /* ALS.opce (is not specified in the encoding's description in
	       C.7.6.2, therefore, stupidly use NONE by analogy with LAS),
	       ALES.opce and ALES.opc2.  */
            printf (", NONE, %s, EXT1", ales_opce[j]);
	    print_final_part ();
	  }
      }
  }
    ...
    for (j = 0; j < 2; j++)
      {
        ...
        for (i = 0; i < sizeof (stores) / sizeof (stores[0]); i++)
          {
            instr_cop_chn cop_chn;
	    const char *cop_name = stores[i].name;
	    const char *opce = "NONE";
	    int real_cpu = mcpu;

	    if (strcmp (cop_name, "stosrrd") == 0
		|| strcmp (cop_name, "stosrrqp") == 0)
	      {
		real_cpu = 7;
		opce = "0xdc";
	      }
	    ...
	    if (strcmp (cop_name, "stosrrd") == 0)
	      cop_name = "strd";
	    else if (strcmp (cop_name, "stosrrqp") == 0)
	      cop_name = "strqp";
	    ...
	    print_common_part_with_cpu (real_cpu, "e2k_alopf13_opcode_templ",
					stores[i].name, "parse_alf_args");
  ...
  for (i = 0; i < 3; i++)
    {
      ...
      instr_cop_chn cop_chn;
      ext_instr_format *crnt = &rr[i];

      if (! fetch_cop_channels (long_opers, crnt->cop_name, &cop_chn))
        continue;

      print_common_part_with_cpu (/* Take into account that RWSHD is supported
				     starting from elbrus-v7 despite the fact
				     that RWD opcode used by it is available
				     for all isets.  */
				  i == 2 ? 7 : mcpu,
      ...
      /* Initialize `e2k_alopf15_opcode_templ.opce'.  */
      printf (", 0x%02x", crnt->dummy);
  ...
  for (i = 0; i < 3; i++)
    {
      ...
      print_common_part_with_cpu (/* Take into account that RRSHD is supported
				     starting from elbrus-v7 despite the fact
				     that RRD opcode used by it is available
				     for all isets.  */
				  i == 2 ? 7 : mcpu,
				  "e2k_alopf16_opcode_templ",
				  crnt->name,
				  "parse_alf_args");
      ...
      /* Initialize `e2k_alopf16_opcode_templ.src2'.  */
      printf (", 0x%02x", crnt->dummy);
  ...
  if (cpu >= 5)
    {
      ...
      if (cpu >= 7)
	{
	  add_combined_table_entry (dst, 1, 6, 0, "qpinss", channels_0134,
				    "ARGS_PSPP");
	  add_combined_table_entry (dst, 1, 6, 1, "qpinsd", channels_0134,
				    "ARGS_PDPP");
	}
    }
}
...
{
  int i;
  int qp = (prefix[0] == 'q' && prefix[1] == 'p') ? 1 : 0;

  /* Starting from elbrus-v7 `QPLOG_*' insns are available in ALC{2,5} in
     addition to the initially supported ALC{0,1,3,4} (see u7.15 in
     iset-v7.single and Bug #88091, Comment #8).  */
  const int channels[] = {
    1, 1, (cpu >= 7 && qp) ? 1 : 0,
    1, 1, (cpu >= 7 && qp) ? 1 : 0
  };
                      ...
                      int first, int last)
{
  ...
  for (i = 0; i < 128; i++)
	...
	lt_pack_code = 1;
      ...
      for (j = 0; j <= lt_pack_code; j++)
	{
	  printf ("{\n\tstatic e2k_alopf21_opcode_templ dummy =");
	  printf ("\n\t\t{\"%s%s\", 0x%02x, parse_alf_args",
		  table->entry[i].name, j == 0 ? "" : "_lt3",
		  /* *_lt3 variants have emerged starting from elbrus-v7.
		     FIXME: this trickery used because of missing *_lt3
		     mnemonics in insn tables makes it impossible to produce
		     a single elbrus-v7 specific entry containing all 6
		     available channels for QPLOG*_lt3.  */
		  j == 0 ? iset_mask : (iset_mask & ~0x3f));

	  print_alf_part (merge ? "ALOPF21_MERGE" : (j == 0
						     ? "ALOPF21"
						     : "ALOPF21_LT_PACK_CODE"),
			  "NO_MAS", i, table->entry[i].channels,
			  table->entry[i].arg_fmts);
      ...
      for (j = 0; j < 7; j++)
        {
          ...
          print_combined_table (opc2[i], &tables[j], j + 1, 7);
        }
    }
}
...
gen_alopf21_custom ()
{
  ...
  static const instr_format instrs[] =
    {
      ...
      {"qpidotsbwss", "ARGS_PPPP"},
      {"qpidotsbwus", "ARGS_PPPP"},
      {"qpidotsbwuu", "ARGS_PPPP"},
      {"qpidotshdss", "ARGS_PPPP"},
      {"qpidotshwss", "ARGS_PPPP"},
  ...
  for (i = 0; i < 3; i++)
    {
      int j;
      const e2k_oper_table *tables[]
	= {long_opers, long_ext1_opers, long_ext2_opers};
      const char *exts[] = {"EXT", "EXT1", "EXT2"};
      for (j = 0; j < sizeof (instrs) / sizeof (instrs[0]); j++)
	{
	  ...
	  /* According to a note below Table B.1.3.1 in iset-v7.single QPMGME
	     may be encoded in ALC0 only.  */
	  if (strcmp (instrs[j].name, "qpmgme") == 0)
	    cop_chn.channels[3] = 0;

	  output_alopf21_insn (instrs[j].name, cop_chn.cop, cop_chn.channels,
			       instrs[j].format, exts[i]);
	  if (strncmp (instrs[j].name, "qptbl_", 6) == 0)
	    {
	      /* Produce the "hex" flavour of the same insn so as to satisfy
		 a rather strange taste of our engineers.  */
	      char hexname[16] = "qptbl_0x";
	      hexname[8] = instrs[j].name[6];
	      hexname[9] = '\0';

	      output_alopf21_insn (hexname, cop_chn.cop,
				   cop_chn.channels,
				   instrs[j].format,
				   exts[i]);
	    }
